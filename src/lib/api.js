...
const ITEM_COLS = 'id, person_id, type, title, date, time, description, refs, contact_id, created_at, tentative, contact:contacts(id, name, specialty, phones, now_serving)'
const itemFromRow = (r) => ({
  id: r.id, personId: r.person_id, type: r.type, title: r.title,
  date: r.date ?? null, time: r.time ?? '', description: r.description ?? '', refs: r.refs ?? [],
  contactId: r.contact_id ?? null, createdAt: r.created_at, tentative: r.tentative ?? false,
...
const itemFields = (it) => ({
  ...
  contact_id: it.contactId ?? null, tentative: !!it.tentative,
})
