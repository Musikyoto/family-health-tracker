...
async function main() {
  ...
  try {
    ...
    ;({ data: { id: contactA } } = await A.from('contacts')
      .insert({ family_id: familyA, name: 'Secret A contact', phones: [{ label: 'Clinic', location: 'St Lukes', number: '0123', days: ['SAT'], hours: '10:00 AM - 12:00 NN' }], now_serving: true }).select('id').single().throwOnError())
